...
export default async function onTerminateInvestment(
  investment: OnTerminateInvestmentData
) {
  const message = (name?: string) =>
    `The investment ${investment.investmentName} (${investment.investmentTier} Tier) was terminated by ${investment.terminator === 'user' && name ? 'you' : investment.terminator === 'user' && !name ? investment.user.name : 'the Invest Tracker admin'} ${!investment.terminationReason ? 'without reason.' : 'with reason: ' + investment.terminationReason}.`
  ...
  const html = (name: string) => {
    return emailTemplate({
      ...
      footer:
        'This message was sent from Invest Tracker because an investment was terminated.'
    })
  }
  ...
  try {
    ...
    logger.error(
      `Error sending investment termination alerts: ${(error as Error).message}`,
      error
    )
  }
}
