...
export default async function onCreateInvestment(
  investment: OnCreateInvestmentData
) {
  const subject = 'New Investment'
  const message = (name?: string) =>
    `A new ${investment.investmentName} (${investment.investmentTier} Tier) was started by ${name ? 'you' : investment.user.name} with an initial deposit of $${investment.initialDeposit.toLocaleString()}`
  ...
  const html = (name?: string) => {
    return emailTemplate({
      ...
      footer:
        'This message was sent from Invest Tracker because a new investment was started.'
    })
  }
  ...
  try {
    ...
    logger.error(
      `Error sending investment creation alerts: ${(error as Error).message}`,
      error
    )
  }
}
